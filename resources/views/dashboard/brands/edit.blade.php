        ...
        <div class="w-full bg-white shadow-md rounded-lg p-4 sm:p-6 lg:p-8 mx-2 sm:mx-4 lg:mx-6 xl:mx-10">
            ...
            <form id="categoryForm" class="w-full" enctype="multipart/form-data">
                ...
                <div class="mb-5">
                    ...
                    <input type="text" id="brand-name" name="brand_name" value="{{ $data->brand_name }}"
                ...
                <div class="mb-5">
                    ...
                    <div id="imagePreview" class="mt-3">
                        <!-- If an existing image exists, show it, else show the preview once a new image is selected -->
                        @if ($data->brand_logo)
                            <img id="previewImg" src="{{ asset('storage/' . $data->brand_logo) }}" alt="Thumbnail Preview"
        ...
        $(document).ready(function() {
            ...
            $('#thumbnail').on('change', function(e) {
                ...
                if (file) {
                    ...
                    // If no file selected, show the existing image (if any)
                    const existingImage = "{{ asset('storage/' . $data->brand_logo) }}";
                    $('#previewImg').attr('src', existingImage).removeClass('hidden');
                }
            });

            // Save the brand using Axios
            saveButton.on('click', function() {
                ...
                const selectedFile = $('#thumbnail')[0].files[0];
                if (selectedFile) {
                    formData.append('brand_logo', selectedFile);
                } else {
                    // If no new image is selected, don't include 'brand_logo' field (existing logo will be retained)
                    formData.append('brand_logo', '{{ $data->brand_logo }}');
                }

                axios.post('{{ route('brands.update', $data->id) }}', formData, {
                    headers: {
